...
test('front desk mode limits the staff tabs to Sales, Bookings & Waitlist', async ({ page }) => {
  ...
  // Switch to Front Desk -> only Sales (cashier) + Bookings (queue) + Waitlist
  await page.evaluate('setStaffRole("frontdesk")');
  expect((await vis()).sort()).toEqual(['cashier', 'queue', 'waitlist']);
