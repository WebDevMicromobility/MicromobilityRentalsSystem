...
// Nobody five or younger, and no birth date in the future. The chooser cannot offer one: the
// years stop six years back, and in that year the months and days still to come are greyed
// out. Every save checks again, which also catches a date stored before the rule.

const today = () => new Date().toLocaleDateString('en-CA', { timeZone: 'Asia/Riyadh' });
const [Y, M, D] = today().split('-').map(Number);
const iso = (y: number, m: number, d: number) => `${y}-${String(m).padStart(2, '0')}-${String(d).padStart(2, '0')}`;
const SIX = iso(Y - 6, M, D);     // six today: the youngest allowed
const FIVE = iso(Y - 5, M, D);    // five today: refused
const YOUNG = 'Riders must be at least 6 years old. Check the birth date.';
...
test('the chooser stops six years back and greys out what is still to come in that year', async ({ page }) => {
  await account(page);
  ...
  expect(Math.max(...years)).toBe(Y - 6);
  ...
  const dim = new Date(Y - 6, M, 0).getDate();
  ...
  await expect(page.locator('#acc-birth')).toHaveValue(SIX);
});
...
test('moving to the last year clears a month that year has not reached', async ({ page }) => {
  ...
  await account(page);
  await page.selectOption('#acc-birth-y', String(Y - 7));
  await page.selectOption('#acc-birth-m', '12');
  await page.selectOption('#acc-birth-d', '31');
  await expect(page.locator('#acc-birth')).toHaveValue(iso(Y - 7, 12, 31));
  await page.selectOption('#acc-birth-y', String(Y - 6));
  await expect(page.locator('#acc-birth-m')).toHaveValue('');
  await expect(page.locator('#acc-birth')).toHaveValue('');
});

test('My Account refuses a rider of five or a future date, and saves one of six', async ({ page }) => {
  ...
  await page.evaluate(`document.getElementById('acc-birth').value='${SIX}';saveAccount()`);
  await expect.poll(() => saves.length).toBe(1);
  expect(saves[0].p_birth_date).toBe(SIX);
});
...
test('the staff account editor holds to the same rule', async ({ page }) => {
  ...
  await page.evaluate(`document.getElementById('cf-birth').value='${SIX}';saveCustForm()`);
  await expect.poll(() => writes.length).toBe(1);
});

test('the rule itself: six today passes, a day younger does not, the future never does', async ({ page }) => {
  await account(page);
  const dayAfter = new Date(Date.UTC(Y - 6, M - 1, D + 1)).toISOString().slice(0, 10);
  expect(await page.evaluate(`[_dobErr(''),_dobErr('${SIX}'),_dobErr('${dayAfter}'),_dobErr('${FIVE}'),_dobErr('2099-01-01'),_dobErr('1990-05-05')]`))
