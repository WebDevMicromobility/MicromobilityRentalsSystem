...
- `staff.micromobility.sa` — the same app on a custom domain of the same Pages project. `_isStaffHost()` locks it to the staff side (sign-in or panel only; `goLanding`/`goCustomer` lead back; `body.staff-host` hides the customer chrome), and `functions/_middleware.js` serves it `noindex` with a deny-all robots.txt.
