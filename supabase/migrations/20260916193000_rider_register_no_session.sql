-- ============================================================================
-- rider_register() without a session no longer errors.
--
-- The RETURN expression names v_win.starts_at / v_win.ends_at inside a CASE that only
-- takes that branch when a session was given, but plpgsql evaluates the record reference
-- first and refuses a record that was never assigned ("record v_win is not assigned yet",
-- SQLSTATE 55000). The form always sends a session, so the desk never saw it; rider_edit()
-- and direct calls can. The record is now given its shape at the top of the function.
-- Only that line differs from 20260916180000.
--
-- APPLIED TO PRODUCTION 2026-09-16 via the Supabase MCP (version 'rider_register_no_session').
-- If the CLI lists this file as pending: supabase migration repair --status applied 20260916193000
-- Rollback: re-run the rider_register definition in 20260916180000_rider_booking_no_uppercase.sql
