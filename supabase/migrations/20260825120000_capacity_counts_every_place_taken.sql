-- ─────────────────────────────────────────────────────────────────────────────
-- A session's capacity means what staff typed: only a CANCELLED booking gives a
-- place back.
--
...
-- cancelled and removed. Checking in does not release one, finishing does not, and a
-- no-show does not either — the rider took a place and did not come, which is a loss
-- rather than a vacancy. That is the rule the client now applies too (_holdsSpot),
-- and the two must agree or the form and the server disagree about who is next.
--
    ...
    if cap is not null then
      ...
      select count(*) into live from queue_entries q
        where q.session_id=new.session_id
          and coalesce(q.status,'') not in ('cancelled','removed')
